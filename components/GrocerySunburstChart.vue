...
<template>
  <div
    v-if="legs.length > 1"
    class="mt-4 w-full rounded-xl border border-emerald-500/30 bg-gradient-to-b from-emerald-950/30 to-slate-950/40 px-3 py-4"
  >
    ...
    <div class="flex flex-col items-center gap-4 sm:flex-row sm:items-start sm:justify-center">
      <svg
        class="shrink-0 drop-shadow-[0_0_20px_rgba(16,185,129,0.15)]"
        :width="box"
        :height="box"
        :viewBox="`0 0 ${box} ${box}`"
        role="img"
      ...
      <ul class="w-full min-w-0 max-w-[16rem] space-y-2 text-[11px] sm:flex-1">
        <li
          v-for="p in outerPaths"
          :key="p.leg.anchorId"
          class="rounded-lg border border-slate-800/80 bg-slate-950/50 px-2.5 py-2"
        >
          <div class="flex items-start justify-between gap-2">
            <span class="flex min-w-0 items-center gap-2">
              <span class="mt-0.5 size-2.5 shrink-0 rounded-sm ring-1 ring-slate-700" :style="{ backgroundColor: p.color }" />
              <span class="truncate font-medium leading-snug text-slate-200">{{ p.leg.name }}</span>
            </span>
          </div>
          <div class="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-slate-500">
            <span
              ><span class="text-slate-600">of groceries</span>
              <span class="tabular-nums font-medium text-slate-400">{{ p.leg.pctOfGrocery.toFixed(1) }}%</span></span
            >
            <span
              ><span class="text-slate-600">of total burden</span>
              <span class="tabular-nums font-medium text-emerald-200/80">{{ p.leg.pctOfTotalBurden.toFixed(1) }}%</span></span
            >
          </div>
        </li>
      </ul>
  ...
  <p
    ...
    class="mt-3 rounded-lg border border-slate-800/80 bg-slate-950/40 px-3 py-2 text-[11px] leading-relaxed text-slate-500"
  >
